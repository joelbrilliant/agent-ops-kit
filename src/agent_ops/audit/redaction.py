...
_TOKEN_RE = re.compile(
    r"(?i)\b(ghp_[A-Za-z0-9]{20,}|gho_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|"
    r"sk-[A-Za-z0-9]{20,}|xox[baprs]-[A-Za-z0-9-]{10,})\b"
)
_ABS_PATH_RE = re.compile(r"(?P<p>/(?:Users|home|var|tmp|private|opt)/[^\s\"']+)")


def redact_text(text: str, extra_substrings: Iterable[str] | None = None) -> str:
    out = text
...
def build_redaction_record(
    *,
    ...
    path_roots_redacted: Iterable[str] | None = None,
) -> Dict[str, Any]:
    return {
...
def assert_no_private_material(text: str, private_markers: Iterable[str] | None = None) -> List[str]:
